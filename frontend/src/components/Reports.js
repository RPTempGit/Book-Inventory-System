...
const Reports = () => {
    ...
    const downloadCSV = () => {
        const csv = [
            ["Type", "Item", "Quantity", "Date"],
            ...transactions.map(t => [t.type, t.item_id?.item_name, t.qty, new Date(t.date).toLocaleDateString()])
        ].map(e => e.join(",")).join("\n")
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="bg-gray-100">
                        <th>Type</th><th>Item</th><th>Qty</th><th>Date</th>
                    </tr>
                </thead>
                <tbody>
                    {transactions.map((t) => (
